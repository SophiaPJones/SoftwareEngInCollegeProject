...
class StateTestCases(TestCase):
    ...
    def test_saving_over_five_accounts(self):
        ...
        newState.users["UserName5"] = User("first","second","Username5", "Password1!")
        newState.users["UserName6"] = User("first","second","Username6", "Password1!")
        newState.users["UserName7"] = User("first","second","Username7", "Password1!")
        newState.users["UserName8"] = User("first","second","Username8", "Password1!")
        newState.users["UserName9"] = User("first","second","Username9", "Password1!")
        newState.users["UserName10"] = User("first","second","Username10", "Password1!")
        newState.users["UserName11"] = User("first","second","Username11", "Password1!")
        newState.save_accounts()
    ...
    def test_search(self):
        ...
        newState.users["user1"] = User('Sophia','Jones','sophie','password1',major = 'Computer Science',university='University of South Florida')
        newState.current_user = newState.users["user1"]
        newState.users["user2"] = User('John','Dinkleburg','dinkster','password2',major = 'Interpretive Dance',university='University of Central Florida')
        newState.users["user3"] = User('Bruce','Wayne','notbatman','password3',major = 'Business Administration',university='Florida Gulf Coast University')
        newState.users["user4"] = User('Peppino','Pizza','pizzafiend','password4',major = 'Culinary Arts',university='School of Hard Knocks')
        newState.users["user5"] = User('Bevin','Barker','angryimpala','password5',major = 'Music Theory',university='University of South Florida')
        assert searchPage.is_exists(lastname='Wayne') == newState.users["user3"]
        assert searchPage.is_exists(lastname = "Jones") == None
        assert searchPage.is_exists(lastname = 'Hoeke') == None
        assert searchPage.is_exists(university='Florida Gulf Coast University') == newState.users["user3"]
        assert searchPage.is_exists(university='Harvard University') == None
        assert searchPage.is_exists(major = 'Culinary Arts') == newState.users["user4"]
